...
5. Subscriptions (early access, scoped on the call): "We support it for you" — we update the actions and pipeline when payloads change and alert you when a plugin breaks. "We host the connection" — for native chat, we run and maintain the connection between Moveworks and ServiceNow (needs ITSM Pro).

Native chat has two delivery options, the customer chooses:
- Built in your instances — the connection lives in your own Moveworks and ServiceNow. You own it outright, no ongoing fee, nothing leaves your tenant. One-time build, quoted after the free call.
- We host the connection — it runs on our infrastructure between your two systems. Subscription, early access. Your Moveworks points at us, we call your ServiceNow with your own service account. ITSM Pro needed either way.
...
- Why use your hosted connection instead of building it in our own instances? Honestly, plenty of customers shouldn't — if your team can maintain the token chain and you'd rather no vendor sat in the path, take the in-tenant build and own it outright. You'd choose the hosted connection when you don't want this to be your team's 3am problem: we maintain it when Moveworks or ServiceNow changes something, we see failures before your users report them, and the external chat window stays configured as your same-day fallback. You are not paying for connectivity — you are paying so it keeps working.
- What happens to our handoff if CLP disappears? The external chat window path is built and kept working as your documented rollback — no extra license, no vendor in the path. If the hosted connection ever went away you flip to it the same day, and the runbook to rebuild the connection in your own instances comes with the engagement.
- Why work with CLP rather than a bigger firm? Both handoff patterns are built and running on real ServiceNow instances — not slideware. We speak on exactly this topic at ServiceNow Nexus for Service Operations and nullEDGE. Our engineering notes are public, so you can judge the depth before you ever talk to us. Fees are fixed in writing before work starts. And the assessment prices staying on Moveworks just as seriously as migrating — we are not paid more for the bigger answer.
